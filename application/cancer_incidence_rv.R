## RAM: 1TB; core: 128; 3 hrs
# updated group specific separable effect function on the original dataset; tod_n added
library(foreach)
library(doParallel)
library(mvtnorm)
library(simsurv)
...
time.grid <- 1
admin.cens <- max(data.df$end.time3)
n <- nrow(data.df)
data.df$a <- data.df$ref_sulf
data.df$end.time <- data.df$end.time3
data.df$event <- data.df$event3
...
competing_inference <- function(npath, train.ueif.a1, train.ueif.a0, estimation.ueif.a1, estimation.ueif.a0, train.time, estimation.time){
  ...
  estimate.a0.gcm <- gcmlcm(x=sort(unique(c(train.time, estimation.time)))[1:max(which(!is.na(colMeans(ueif.a0))))], y=colMeans(ueif.a0)[1:max(which(!is.na(colMeans(ueif.a0))))], type="gcm")[c("x.knots","y.knots")]
  names(estimate.a0.gcm) <- c("t", "estimate.a0")
  ueif.df <- merge(data.frame(t=sort(unique(c(train.time, estimation.time)))), estimate.a0.gcm, all=TRUE, by="t")
  ...
  estimate.a1.gcm <- gcmlcm(x=sort(unique(c(train.time, estimation.time)))[1:max(which(!is.na(colMeans(ueif.a1))))], y=colMeans(ueif.a1)[1:max(which(!is.na(colMeans(ueif.a1))))], type="gcm")[c("x.knots","y.knots")]
...
train.Sj.super.a0 <- survSuperLearner(time = estimation.df$end.time[estimation.df$a==0], event = 1*(estimation.df$event[estimation.df$a==0]==1), X = estimation.df[estimation.df$a==0, confounders], cvControl = list(V = 2L, stratifyCV = FALSE, shuffle = TRUE, validRows = NULL),
                                      newX = train.df[, confounders], new.times = sort(unique(train.df$end.time)), event.SL.library = event.sl.lib, cens.SL.library = cens.sl.lib, verbose = FALSE)
train.Sj.a0 <- train.Sj.super.a0$event.SL.predict


train.Sj.super.a1 <- survSuperLearner(time = estimation.df$end.time[estimation.df$a==1], event = 1*(estimation.df$event[estimation.df$a==1]==1), X = estimation.df[estimation.df$a==1, confounders], cvControl = list(V = 2L, stratifyCV = FALSE, shuffle = TRUE, validRows = NULL),
                                          newX = train.df[, confounders], new.times = sort(unique(train.df$end.time)), event.SL.library = event.sl.lib, cens.SL.library = cens.sl.lib, verbose = FALSE)
...
# conditional competing
estimation.Sj.super.a0 <- survSuperLearner(time = train.df$end.time[train.df$a==0], event = 1*(train.df$event[train.df$a==0]==1), X = train.df[train.df$a==0, confounders], cvControl = list(V = 2L, stratifyCV = FALSE, shuffle = TRUE, validRows = NULL),
                                           newX = estimation.df[, confounders], new.times = sort(unique(estimation.df$end.time)), event.SL.library = event.sl.lib, cens.SL.library = cens.sl.lib, verbose = FALSE)
estimation.Sj.a0 <- estimation.Sj.super.a0$event.SL.predict

estimation.Sj.super.a1 <- survSuperLearner(time = train.df$end.time[train.df$a==1], event = 1*(train.df$event[train.df$a==1]==1), X = train.df[train.df$a==1, confounders], cvControl = list(V = 2L, stratifyCV = FALSE, shuffle = TRUE, validRows = NULL),
                                           newX = estimation.df[, confounders], new.times = sort(unique(estimation.df$end.time)), event.SL.library = event.sl.lib, cens.SL.library = cens.sl.lib, verbose = FALSE)
...
start.time <- Sys.time()
f.sim$dr.direct.mc.ha.driptw <- competing_inference(npath=npath, train.ueif.a1=f.train$dr.direct.mc.ha.driptw.ueif$ueif.a1, train.ueif.a0=f.train$dr.direct.mc.ha.driptw.ueif$ueif.a0,
...
f.sim$dr.direct.mc.ha.drow <- competing_inference(npath=npath, train.ueif.a1=f.train$dr.direct.mc.ha.drow.ueif$ueif.a1, train.ueif.a0=f.train$dr.direct.mc.ha.drow.ueif$ueif.a0,
...
save(f.sim, file="cancer_incidence_rv.RData")
