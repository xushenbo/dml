...
# outcome regression rmst by integrating survival
or_rmst_integral_point <- function(id, a, time, event, tilt, S.a0, S.a1, freq.time=NULL, admin.cens)
{
  n <- length(id)
  ...
  estimate.a0 <- 1/mean(tilt)*cumsum(cummin(colMeans(term2.a0))*ds)
  estimate.a1 <- 1/mean(tilt)*cumsum(cummin(colMeans(term2.a1))*ds)
  return(data.frame(s, estimate.a0, estimate.a1))
}

# ipcw rmst by integrating survival
ipcw_rmst_integral <- function(id, a, time, event, bw, G.a0, G.a1, freq.time=NULL, admin.cens)
{
  n <- length(id)
  if(is.null(freq.time)) {s <- sort(unique(time))} else{
    s <- seq(freq.time, admin.cens, freq.time)}
  ...
  estimate.a0 <- 1/mean(bw*(a==0))*cumsum(cummin(colMeans(term1.a0))*ds)
  estimate.a1 <- 1/mean(bw*(a==1))*cumsum(cummin(colMeans(term1.a1))*ds)
  return(data.frame(s, estimate.a0, estimate.a1))
}

# dr rmst by integrating survival
dr_rmst_integral <- function(id, a, time, event, bw, tilt, G.a0, G.a1, S.a0, S.a1, freq.time=NULL, admin.cens)
{
  n <- length(id)
  if(is.null(freq.time)) {s <- sort(unique(time))} else{
    s <- seq(freq.time, admin.cens, freq.time)}
  ...
  estimate.a0 <- cumsum((cummin(1/mean(bw*(a==0))*colMeans(term1.a0+term3.a0)+1/mean(tilt)*colMeans(term2.a0)))*ds)
  # 1/mean(bw*(a==0))*cumsum(cummin(colMeans(term1.a0+term3.a0))*ds)+1/mean(tilt)*cumsum(cummin(colMeans(term2.a0))*ds)
  ...
  estimate.a1 <- cumsum((cummin(1/mean(bw*(a==1))*colMeans(term1.a1+term3.a1)+1/mean(tilt)*colMeans(term2.a1)))*ds)
  # 1/mean(bw*(a==1))*cumsum(cummin(colMeans(term1.a1+term3.a1))*ds)+1/mean(tilt)*cumsum(cummin(colMeans(term2.a1))*ds)
  return(data.frame(s, estimate.a0, estimate.a1))
}
...
for(i in 1:nsim) {
  ...
  S.a0 <- predictSurvProb(coxph(Surv(end.time, 1*(event==1))~x1+x2+x3+x4+x5+x6, data=sim.df[sim.df$a==0,], x=TRUE), newdata=sim.df, times=sort(unique(sim.df$end.time)))
  S.a1 <- predictSurvProb(coxph(Surv(end.time, 1*(event==1))~x1+x2+x3+x4+x5+x6, data=sim.df[sim.df$a==1,], x=TRUE), newdata=sim.df, times=sort(unique(sim.df$end.time)))
  ...
  f.sim$or.integral <- or_rmst_integral_point(id=sim.df$id, a=sim.df$a, time=sim.df$end.time, event=sim.df$event, tilt=sim.df$naive, S.a0=S.a0, S.a1=S.a1)
  f.sim$or.integral.ow <- or_rmst_integral_point(id=sim.df$id, a=sim.df$a, time=sim.df$end.time, event=sim.df$event, tilt=sim.df$ow.tilt, S.a0=S.a0, S.a1=S.a1)
  f.sim$or.integral.mw <- or_rmst_integral_point(id=sim.df$id, a=sim.df$a, time=sim.df$end.time, event=sim.df$event, tilt=sim.df$mw.tilt, S.a0=S.a0, S.a1=S.a1)
  f.sim$or.integral.enw <- or_rmst_integral_point(id=sim.df$id, a=sim.df$a, time=sim.df$end.time, event=sim.df$event, tilt=sim.df$enw.tilt, S.a0=S.a0, S.a1=S.a1)
  
  f.sim$ipcw.integral.iptw <- ipcw_rmst_integral(id=sim.df$id, a=sim.df$a, time=sim.df$end.time, event=sim.df$event, bw=sim.df$iptw, G.a0=G.a0, G.a1=G.a1)
  f.sim$ipcw.integral.iptw.trunc <- ipcw_rmst_integral(id=sim.df$id, a=sim.df$a, time=sim.df$end.time, event=sim.df$event, bw=sim.df$iptw.trunc, G.a0=G.a0, G.a1=G.a1)
  f.sim$ipcw.integral.ow <- ipcw_rmst_integral(id=sim.df$id, a=sim.df$a, time=sim.df$end.time, event=sim.df$event, bw=sim.df$ow, G.a0=G.a0, G.a1=G.a1)
  f.sim$ipcw.integral.mw <- ipcw_rmst_integral(id=sim.df$id, a=sim.df$a, time=sim.df$end.time, event=sim.df$event, bw=sim.df$mw, G.a0=G.a0, G.a1=G.a1)
  f.sim$ipcw.integral.enw <- ipcw_rmst_integral(id=sim.df$id, a=sim.df$a, time=sim.df$end.time, event=sim.df$event, bw=sim.df$enw, G.a0=G.a0, G.a1=G.a1)
  
  f.sim$dr.integral.driptw <- dr_rmst_integral(id=sim.df$id, a=sim.df$a, time=sim.df$end.time, event=sim.df$event, bw=sim.df$iptw, tilt=sim.df$naive, G.a0=G.a0, G.a1=G.a1, S.a0=S.a0, S.a1=S.a1)
  f.sim$dr.integral.driptw.trunc <- dr_rmst_integral(id=sim.df$id, a=sim.df$a, time=sim.df$end.time, event=sim.df$event, bw=sim.df$iptw.trunc, tilt=sim.df$naive, G.a0=G.a0, G.a1=G.a1, S.a0=S.a0, S.a1=S.a1)
  f.sim$dr.integral.drow <- dr_rmst_integral(id=sim.df$id, a=sim.df$a, time=sim.df$end.time, event=sim.df$event, bw=sim.df$ow, tilt=sim.df$ow.tilt, G.a0=G.a0, G.a1=G.a1, S.a0=S.a0, S.a1=S.a1)
  f.sim$dr.integral.drmw <- dr_rmst_integral(id=sim.df$id, a=sim.df$a, time=sim.df$end.time, event=sim.df$event, bw=sim.df$mw, tilt=sim.df$mw.tilt, G.a0=G.a0, G.a1=G.a1, S.a0=S.a0, S.a1=S.a1)
  f.sim$dr.integral.drenw <- dr_rmst_integral(id=sim.df$id, a=sim.df$a, time=sim.df$end.time, event=sim.df$event, bw=sim.df$enw, tilt=sim.df$enw.tilt, G.a0=G.a0, G.a1=G.a1, S.a0=S.a0, S.a1=S.a1)
  ...
  f.sim.boot <- foreach(j=1:nboot,.combine=cbind) %dopar% {
    ...
    f.sim$or.integral <- or_rmst_integral_point(id=sample.df$id, a=sample.df$a, time=sample.df$end.time, event=sample.df$event, tilt=sample.df$naive, S.a0=S.a0, S.a1=S.a1)
    f.sim$or.integral.ow <- or_rmst_integral_point(id=sample.df$id, a=sample.df$a, time=sample.df$end.time, event=sample.df$event, tilt=sample.df$ow.tilt, S.a0=S.a0, S.a1=S.a1)
    f.sim$or.integral.mw <- or_rmst_integral_point(id=sample.df$id, a=sample.df$a, time=sample.df$end.time, event=sample.df$event, tilt=sample.df$mw.tilt, S.a0=S.a0, S.a1=S.a1)
    f.sim$or.integral.enw <- or_rmst_integral_point(id=sample.df$id, a=sample.df$a, time=sample.df$end.time, event=sample.df$event, tilt=sample.df$enw.tilt, S.a0=S.a0, S.a1=S.a1)
    
    f.sim$ipcw.integral.iptw <- ipcw_rmst_integral(id=sample.df$id, a=sample.df$a, time=sample.df$end.time, event=sample.df$event, bw=sample.df$iptw, G.a0=G.a0, G.a1=G.a1)
    f.sim$ipcw.integral.iptw.trunc <- ipcw_rmst_integral(id=sample.df$id, a=sample.df$a, time=sample.df$end.time, event=sample.df$event, bw=sample.df$iptw.trunc, G.a0=G.a0, G.a1=G.a1)
    f.sim$ipcw.integral.ow <- ipcw_rmst_integral(id=sample.df$id, a=sample.df$a, time=sample.df$end.time, event=sample.df$event, bw=sample.df$ow, G.a0=G.a0, G.a1=G.a1)
    f.sim$ipcw.integral.mw <- ipcw_rmst_integral(id=sample.df$id, a=sample.df$a, time=sample.df$end.time, event=sample.df$event, bw=sample.df$mw, G.a0=G.a0, G.a1=G.a1)
    f.sim$ipcw.integral.enw <- ipcw_rmst_integral(id=sample.df$id, a=sample.df$a, time=sample.df$end.time, event=sample.df$event, bw=sample.df$enw, G.a0=G.a0, G.a1=G.a1)
    
    f.sim$dr.integral.driptw <- dr_rmst_integral(id=sample.df$id, a=sample.df$a, time=sample.df$end.time, event=sample.df$event, bw=sample.df$iptw, tilt=sample.df$naive, G.a0=G.a0, G.a1=G.a1, S.a0=S.a0, S.a1=S.a1)
    f.sim$dr.integral.driptw.trunc <- dr_rmst_integral(id=sample.df$id, a=sample.df$a, time=sample.df$end.time, event=sample.df$event, bw=sample.df$iptw.trunc, tilt=sample.df$naive, G.a0=G.a0, G.a1=G.a1, S.a0=S.a0, S.a1=S.a1)
    f.sim$dr.integral.drow <- dr_rmst_integral(id=sample.df$id, a=sample.df$a, time=sample.df$end.time, event=sample.df$event, bw=sample.df$ow, tilt=sample.df$ow.tilt, G.a0=G.a0, G.a1=G.a1, S.a0=S.a0, S.a1=S.a1)
    f.sim$dr.integral.drmw <- dr_rmst_integral(id=sample.df$id, a=sample.df$a, time=sample.df$end.time, event=sample.df$event, bw=sample.df$mw, tilt=sample.df$mw.tilt, G.a0=G.a0, G.a1=G.a1, S.a0=S.a0, S.a1=S.a1)
    f.sim$dr.integral.drenw <- dr_rmst_integral(id=sample.df$id, a=sample.df$a, time=sample.df$end.time, event=sample.df$event, bw=sample.df$enw, tilt=sample.df$enw.tilt, G.a0=G.a0, G.a1=G.a1, S.a0=S.a0, S.a1=S.a1)
...
save(f.sim.result, file="sim2-baseline.RData") # , f.sim.result.tau6
